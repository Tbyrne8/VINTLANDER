export default function ControlPointLocator({ point, mapCenter, zoom, index = 0 }) {
  if (!point?.position || !mapCenter) return null;

  const offset = getOffsetMetres(mapCenter, point.position);
  const range = Math.hypot(offset.eastMetres, offset.northMetres);
  const viewRadius = getApproxViewRadiusMetres(zoom);

  if (range < viewRadius * 0.78) {
    return null;
  }

  const bearing = getBearingDegrees(offset.eastMetres, offset.northMetres);
  const stagger = (index % 4) * 2;
  const left =
    50 + (offset.eastMetres / Math.max(range, 1)) * 43 + stagger;
  const top =
    50 - (offset.northMetres / Math.max(range, 1)) * 43 + stagger;

  return (
    <div
      className={`controlPointLocator ${point.type}`}
      style={{
        left: `${clamp(left, 7, 93)}%`,
        top: `${clamp(top, 8, 92)}%`,
      }}
      title={`${point.name} ${formatDistance(range)} ${bearing
        .toString()
        .padStart(3, "0")} deg`}
    >
